...
import { INGESTION_WARNINGS_OUTPUT, IngestionOutputConfig } from '../../../ingestion/event-processing/ingestion-outputs'
...
import { PersonMessage } from './repositories/person-repository'
...
export class PersonMergeService {
    ...
    constructor(private context: PersonContext) {
        ...
        this.ingestionWarningsOutput = context.outputs.resolve(INGESTION_WARNINGS_OUTPUT)
    }
    ...
    private async mergeDistinctIds(
        ...
        if ((otherPerson && !mergeIntoPerson) || (!otherPerson && mergeIntoPerson)) {
            ...
            return await this.context.personStore.inTransaction('mergeDistinctIds-OneExists', async (tx) => {
                ...
                await this.context.outputs.produceMessages(kafkaMessages)
    ...
    private async executeTransaction(
        ...
        try {
            ...
            const kafkaAck = this.context.outputs.produceMessages(kafkaMessages)
